...
<head>
    ...
    <style>
        ...
        /* ألوان الحالات */
        .status-pending { border-top: 6px solid #ffc107; }
        .status-preparing { border-top: 6px solid #0dcaf0; }
        ...
        .btn-start { background-color: #0dcaf0; color: #000; }
        .btn-start:hover { background-color: #0bb5d9; }
        
        .btn-done { background-color: #198754; color: white; }
        .btn-done:hover { background-color: #157347; }
                ...
                <div class="card-header-status">
                    <div>
                        <h5 class="mb-0 fw-bold">
                            @if($order->table_id)
                                <i class="fas fa-chair text-muted me-1"></i> طاولة {{ $order->table->table_number }}
                            @else
                    ...
                    <span class="badge {{ $order->status == 'pending' ? 'bg-warning text-dark' : 'bg-info text-dark' }} px-3 py-2">
...
<script>
    // تحديث الصفحة تلقائياً كل دقيقة لجعل التوقيت diffForHumans محدثاً
    setTimeout(function(){
       location.reload();
    }, 60000);
</script>
