    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="card bg-dark border-secondary shadow-lg mb-4" style="border-radius: 20px;">
                <div class="card-header border-secondary bg-secondary text-white fw-bold d-flex justify-content-between py-3">
                ...
                <div class="card-body p-4">
                    <div class="p-3 mb-4 rounded-4 bg-info bg-opacity-10 border border-info border-opacity-25 d-flex justify-content-between align-items-center">
                        <div>
                            <div class="text-info fw-bold mb-1"><i class="fas fa-warehouse me-2"></i> الرصيد الحالي بالمستودع</div>
                            <small class="text-muted">بيانات حقيقية مستمدة من سجلات المخزن</small>
                        </div>
                        <div class="text-start">
                            <span class="h2 fw-bold mb-0 {{ $item->quantity <= $item->min_quantity ? 'text-danger' : 'text-success' }}">
                    ...
                    <div class="row g-4 text-white">
                        ...
                        <div class="col-sm-6">
                            ...
                            <span class="badge bg-primary px-3 py-2 rounded-pill mt-1">{{ $item->category->name }}</span>
                        </div>
                        <div class="col-sm-6">
                            <label class="text-muted small d-block">وقت التحضير</label>
                            <span class="fw-bold"><i class="far fa-clock text-info me-1"></i> {{ $item->prepare_time ?? '15' }} دقيقة</span>
                        </div>
                    </div>
                ...
                <div class="card-footer border-secondary bg-black bg-opacity-25 d-flex gap-2 p-3">
                    <a href="{{ route('Pages.Items.edit', $item->id) }}" class="btn btn-warning text-dark px-4 fw-bold rounded-pill shadow-sm">
                        <i class="fas fa-edit me-1"></i> تعديل
                    </a>
                    <form action="{{ route('Pages.Items.destroy', $item->id) }}" method="POST" onsubmit="return confirm('حذف الطبق يزيله من المخزن أيضاً. استمرار؟')">
                        @csrf @method('DELETE')
