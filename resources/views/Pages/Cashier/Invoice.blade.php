...
<head>
    ...
    <style>
        @media print {
    /* إخفاء كل عناصر الصفحة (الهيدر، الأزرار، الخلفية) */
    body * {
        visibility: hidden;
    }
    
    /* إظهار منطقة الفاتورة فقط (المودال أو جدول التفاصيل) */
    .printable-area, .printable-area * {
        visibility: visible;
        color: #000 !important; /* تحويل النص للأسود للطباعة */
    }

    /* تنسيق الفاتورة لتناسب الورق الحراري */
    .printable-area {
        position: absolute;
        left: 0;
        top: 0;
        width: 80mm; /* مقاس ورق الطابعات الحرارية القياسي */
        background: #fff !important;
        padding: 10px;
    }

    /* إخفاء الأزرار داخل الفاتورة عند الطباعة */
    .no-print {
    ...
    @page {
        ...
        .table { color: #e6e8ed; margin-bottom: 0; border-color: var(--border-dim); }
        .table thead th {
            background: #242938;
        ...
        .table tbody td { padding: 15px; vertical-align: middle; border-bottom: 1px solid var(--border-dim); }

        /* --- تنسيق أيقونة الـ Show (بدون سكريبت) --- */
        .view-link {
            width: 42px;
            height: 42px;
            display: inline-flex;
        ...
        /* حل مشكلة الأسهم الضخمة في الـ Pagination */
        .pagination svg {
    ...
    <div class="row mb-4 align-items-center">
        ...
        <div class="col-md-6 text-md-end mt-3 mt-md-0">
            <form action="" method="GET" class="d-flex gap-2 justify-content-md-end">
                <input type="date" name="date" class="form-control bg-dark border-secondary text-white w-auto" value="{{ request('date') ?? date('Y-m-d') }}">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <span class="badge bg-secondary bg-opacity-10 text-white border border-secondary px-3 py-2">
                                    سفري (POS)
                                </span>
                            @endif
                        </td>
                        ...
                        <td>
                            <div class="d-flex justify-content-center gap-2">
                                <a href="{{ route('Pages.invoice.show', $invoice->id) }}" class="view-link" title="عرض التفاصيل">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-5 opacity-50">لا توجد سجلات مالية لهذا اليوم</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
