<div class="row g-4 text-white">
    <div class="col-md-6">
        ...
        <input type="number" name="table_number"
            ...
            placeholder="@lang('For example: 1')" value="{{ old('table_number', $table->table_number ?? '') }}" required>
    </div>
    ...
    <div class="col-md-6">
        ...
        <select name="status" class="form-select bg-secondary bg-opacity-10 border-secondary text-white shadow-none">
            <option value="available" {{ old('status', $table->status ?? '') == 'available' ? 'selected' : '' }}>@lang('Available')</option>
            <option value="reserved" {{ old('status', $table->status ?? '') == 'reserved' ? 'selected' : '' }}>@lang('Reserved')</option>
            @if(isset($table))
                <option value="occupied" {{ old('status', $table->status ?? '') == 'occupied' ? 'selected' : '' }}>@lang('Occupied')</option>
            @endif
        </select>
    </div>
</div>