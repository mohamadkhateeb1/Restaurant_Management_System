...
<div class="container mt-5">
        ...
        <div class="col-md-6 text-center text-md-end mt-4 mt-md-0">
            <div class="d-flex gap-2 justify-content-center justify-content-md-end">
                ...
                @if($categories->count() > 0)
                    <button type="button" class="btn btn-outline-danger shadow-sm px-4 rounded-pill transition-all" onclick="confirmDeleteAll()">
                        <i class="fas fa-trash-sweep me-2"></i> مسح القائمة
                    </button>
                @endif
                
                <a href="{{ route('Pages.categories.create') }}" class="btn btn-primary shadow-sm px-4 rounded-pill transition-all hover-lift">
    ...
    <form id="delete-all-form" action="{{ route('Pages.categories.bulkDestroy') }}" method="POST" style="display:none;">
        @csrf
        @method('DELETE')
                        ...
                        <tr class="border-bottom border-secondary transition-all">
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="img-container me-3 position-relative">
                                        ...
                                        @else
                                            <div class="bg-secondary bg-opacity-25 rounded-3 d-flex align-items-center justify-content-center" style="width: 60px; height: 60px;">
                                    ...
                                    <div>
                                        ...
                                        <small class="text-muted d-block text-truncate" style="max-width: 150px;">{{ $category->description ?? 'لا يوجد وصف' }}</small>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="price-tag">
                                    <span class="text-warning fw-bold h5 mb-0">{{ number_format($category->price, 2) }}</span>
                                    <small class="text-muted opacity-50 ms-1">د.أ</small>
                                </div>
                            </td>
                            ...
                            <td class="pe-4">
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('Pages.categories.show', $category->id) }}" class="btn btn-icon btn-soft-primary" title="عرض"><i class="fas fa-eye"></i></a>
                                    <a href="{{ route('Pages.categories.edit', $category->id) }}" class="btn btn-icon btn-soft-warning" title="تعديل"><i class="fas fa-edit"></i></a>
                                    
                                    <form action="{{ route('Pages.categories.destroy', $category->id) }}" method="POST" class="d-inline" onsubmit="return confirm('هل تريد حذف هذا الصنف نهائياً؟')">
                                        @csrf
                                        @method('DELETE')
...
<style>
    ...
    .btn-icon {
        ...
        border-radius: 50%; border: none; transition: 0.3s;
    }

    /* تصميم حالات الأزرار الناعمة */
    .btn-soft-primary { background: rgba(13, 110, 253, 0.1); color: #0d6efd; }
    .btn-soft-primary:hover { background: #0d6efd; color: #fff; box-shadow: 0 4px 10px rgba(13, 110, 253, 0.3); }
    
    .btn-soft-warning { background: rgba(255, 193, 7, 0.1); color: #ffc107; }
    .btn-soft-warning:hover { background: #ffc107; color: #000; box-shadow: 0 4px 10px rgba(255, 193, 7, 0.3); }
    
    .btn-soft-danger { background: rgba(220, 53, 69, 0.1); color: #dc3545; }
    .btn-soft-danger:hover { background: #dc3545; color: #fff; box-shadow: 0 4px 10px rgba(220, 53, 69, 0.3); }
    ...
    .status-active .status-dot { background: #28a745; box-shadow: 0 0 8px #28a745; }
    ...
    .empty-icon-container { position: relative; display: inline-block; }
